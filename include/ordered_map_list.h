...
template<typename T>
class OrderedMapList {
private:
	int current;
	ForwardList<pair<int, Polynomial<T>>> table;
...
public:
	OrderedMapList() = default;
	OrderedMapList(const OrderedMapList& map) = default;
	OrderedMapList(OrderedMapList&& map) = default;
	~OrderedMapList() = default;
	OrderedMapList& operator=(const OrderedMapList& map) = default;
	OrderedMapList& operator=(OrderedMapList&& map) = default;
	...
	bool operator==(const OrderedMapList& map) const {
		size_t size = Size();
		for (size_t i = 0; i < size; i++) {
